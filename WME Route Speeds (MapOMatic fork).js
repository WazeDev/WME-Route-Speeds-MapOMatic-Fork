...
/*Version history:
 * 2021.10.17.001
    - Update for routing data changes - thanks to Timbones for a critical formula
...
//        var routespeedsoption15 = false;  // ***temporarily disabled
...
//            var obj15 = getId('routespeeds-option15');  // *** temporarily disabled
...
//                localStorage.setItem("RouteSpeedsOption15", obj15.checked);  // ***temporarily disabled
...
//            if (localStorage.RouteSpeedsOption15) routespeedsoption15 = (localStorage.RouteSpeedsOption15 == "true");  // *** temporarily disabled
...
//            getId('routespeeds-option15').checked = routespeedsoption15;  // *** temporarily disabled
            ...
            if (markerA === undefined && markerB === undefined) {
                ...
                markerA.dragging.done = function (e) {
                    ...
                    if (getId('sidepanel-routespeeds-a') !== undefined) {
                        getId('sidepanel-routespeeds-a').value = lat1 + ", " + lon1;
                        getId('sidepanel-routespeeds-b').value = lat2 + ", " + lon2;
                    }
        ...
        function createRouteFeatures(id, routewsp, routeodc) {
            ...
            var odctime = 0;
            if (routespeedsoption7) odctime = routeodc[odc].cross_time;
            else odctime = routeodc[odc].cross_time_without_real_time;
            var odcx = 0;
            var odcy = 0;
            ...
            if (doubletraffic) {
                doubletrafficoffset = 11 * Math.pow(2.0, 5 - W.map.getZoom() + 12); //Zoom fix Sep 2021
            ...
            if (routespeedsoption7) {
                for (let i = 0; i < routeodc.length; i++) {
                    ...
                    summarysec += routeodc[i].cross_time;
                }
            }
            else {
                for (let i = 0; i < routeodc.length; i++) {
                    summarylen += routeodc[i].length;
                    summarysec += routeodc[i].cross_time_without_real_time;
                }
            }
        ...
        function requestRouteFromLiveMap(x1, y1, x2, y2) {
            ...
            var numPaths = numRoutes;  // leftover from Try More option - probably want to clean up some day
    ...
    /*					if (json.coords !== undefined) {
                            ...
                            routeodc1 = json.response.result;
...
*/
                            ...
                            if (json.response.length > numRoutes) {
                                json.response = json.response.slice(0, numRoutes);
                            }

                            if (routeSelectedLast) routeSelected = routeSelectedLast;
                            if (routeSelected > json.response.length) routeSelected = json.response.length;

                            for (let n = 0; n < json.response.length; n++) {

                                if (n === 0) routewsp1 = OpenLayers.Geometry.fromWKT(json.response[n].geom.wkt).getVertices();
                                if (n === 0) routeodc1 = json.response[n].result;


                                if (n === 1) routewsp2 = OpenLayers.Geometry.fromWKT(json.response[n].geom.wkt).getVertices();
                                if (n === 1) routeodc2 = json.response[n].result;

                                if (n === 2) routewsp3 = OpenLayers.Geometry.fromWKT(json.response[n].geom.wkt).getVertices();
                                if (n === 2) routeodc3 = json.response[n].result;

                                if (n === 3) routewsp4 = OpenLayers.Geometry.fromWKT(json.response[n].geom.wkt).getVertices();
                                if (n === 3) routeodc4 = json.response[n].result;

                                if (n === 4) routewsp5 = OpenLayers.Geometry.fromWKT(json.response[n].geom.wkt).getVertices();
                                if (n === 4) routeodc5 = json.response[n].result;
                            }
                        }
...
//            getId('routespeeds-option15').checked = routespeedsoption15 = true; // *** temporarily disabled
        ...
        function gotoMarker(marker) {
            ...
            let zoom = window.W.map.getZoom() + 12;
...
/*        function clickOption15() {
            routespeedsoption15 = (getId('routespeeds-option15').checked === true);  // *** temporarily disabled
            livemapRoute();
        }
*/        //--------------------------------------------------------------------------------------------------------
...
//                    { display: 'inline-block' }, { marginLeft: '10px', paddingLeft: '17px' }) +
                ...
                getCheckboxHtml('option7', 'Real-Time Traffic', 'note: this only seems to affect routes within the previous 30-60 minutes') +
//                getCheckboxHtml('option15', 'Use Routing Order', 'Sorts routes in the same order they would appear in the app or livemap') +   // *** temporarily disabled
...
//            getId('routespeeds-option15').onclick = clickOption15; // *** temporarily disabled
